...
extension BinarySearchTree {
    ...
    public func searchRecursive(value: T) -> BinarySearchTree? {
        if value < self.value { // not quite the droid youre looking for
            return left?.searchRecursive(value: value)
        } else if value > self.value { // still not quite right...
            return right?.searchRecursive(value: value)
        } else { // You've found the droid you're looking for!
            return self
        }
    }
...
/*
 NOTE: If there are duplicate items in the tree, search() returns the "highest" node because we start searching from the root downwards.
*/
let tree = BinarySearchTree<Int>(array: [7, 2, 5, 10, 12, 9])
